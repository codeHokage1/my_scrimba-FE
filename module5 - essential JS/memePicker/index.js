...
const radios = document.getElementsByName('emotion');

const memeBtn = document.getElementById('get-image-btn');
const wantGIF = document.getElementById('gifs-only-option');

const modal = document.getElementById('meme-modal');
const modalContainer = document.getElementById('meme-modal-inner');
...
memeBtn.addEventListener("click", () => {
    ...
    for (let data of catsData) {
        if (data.emotionTags.includes(selected) && data.isGif === wantGIF.checked) {
            imageArrays.push([data.image, data.alt])
        }
    }
    console.log(imageArrays)
    const randomImage = imageArrays[Math.floor(Math.random() * imageArrays.length)];
    const image = `<img src="images/${randomImage[0]}" alt="${randomImage[1]}" class="cat-img">`
    modalContainer.innerHTML = image;
